...
from numpy import concatenate, arcsin,arctan2
...
def get_stuffing_coils(a1,a2,b1,b2,plot=False):
    coil_a1 = get_stuff_coil(dx=a1[0], dy=a1[1],r= .02, d=.08, nturns=12)
...
def get_stuff_coil(dx=0, dy=0, dz=0, r=.02, d=.10, nturns=208.):
    theta = arange(0,2*pi*nturns,pi/10.)
    path = array([r*cos(theta)+dx,r*sin(theta)+dy,-d*theta/(2*pi*nturns)+dz])
    return path
...
def plot_all(plot_candelabra=True,plot_coils=False,plot_loops=False):
    '''Plot electrodes'''
    ...
    if plot_coils:
        get_stuffing_coils(loop_A_upper_pos,loop_A_lower_pos,loop_B_upper_pos,loop_B_lower_pos,plot=True)
...
if __name__ == "__main__":
    ...
    x = radius*cos(theta)
    y = radius*sin(theta) - 9.75*.0254
    mlab.mesh(x, y, z, color=(0.74609375,0.41015625,0.20703125))
    ...
    ##B-field background
    m=-1;x0=b1[0];y0=b1[1]
    X, Y, Z = mgrid[-9:9,-20:10,0:12]/50. # meters
